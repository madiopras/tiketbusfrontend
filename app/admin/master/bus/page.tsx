...
import UserTable from "../../components/TableBus";
...
const UserListPage = () => {
  ...
  const fetchClassNames = useCallback(async () => {
    try {
      ...
      const formattedClassOptions = response.data.data.map((classItem: { class_name: any; }) => ({
  ...
  const handleView = (id: number) => {
    router.push(`/admin/master/buses/view/${id}`);
  };
